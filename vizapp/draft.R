...
foldername = "trackIdentity/"
filename = paste0(root,foldername,"graph.csv")
thresholdf = 5
thresholdp = 1000
...
DefineNetWork = function(nodesdf,edgesdf){
  net = visNetwork(nodesdf, edgesdf, height = "5000px", width = "100%") %>%
...
df = CleanDf(filename,thresholdf,thresholdp)
...
net = DefineNetWork(nodesdf,edgesdf)
print(net)
